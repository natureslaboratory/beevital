...
add_action('wp_enqueue_scripts', function () {
    ...
    wp_enqueue_style('bv', get_bloginfo('stylesheet_directory') . '/style.css?v=0.0.8');
...
add_action('wp_footer', 'script_in_footer', 1000);

function script_in_footer()
{
    ?>
    <script>
        const testSelectDiv = document.getElementById("test");

        if (testSelectDiv) {
            const select = document.createElement("select");
            const options = ["hello", "there", "general", "kenobi"];
            const optionsElements = options.forEach(o => {
                const option = document.createElement("option")
                option.value = o;
                option.innerHTML = o;
                select.appendChild(option);
            })
            testSelectDiv.appendChild(select);
        }
    </script>
<?php
}
